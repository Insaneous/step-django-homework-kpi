<x-layout class="overflow-hidden">
    <div class="bg-white text-black rounded-xl w-[80vw] max-h-[88vh] mt-20">
        <div class="flex gap-4 items-center px-1 pt-1">
            ...
            <div class="rounded-t-xl bg-blue-600 p-4 text-white">Departments</div>
            <a href="{{ route('admin-tasks', 1) }}">Tasks</a>
        </div>
        <div class="overflow-y-scroll max-h-[80vh]">
            <x-form class="text-white" :action="route('edit-department', $department->id)" method="POST">
                @csrf
                @method('PUT')
                <div class="flex flex-col">
                    <label for="name">Name</label>
                    <x-input type="text" name="name" value='{{$department->name}}' />
                </div>
                <x-button class="mt-6">Edit</x-button>
            </x-form>
            <form class="flex w-[100%] px-4">
                <x-input type="text" placeholder="Search department" class="w-[100%] pr-8" name='search' />
            ...
            <div class="p-4">
                ...
                @foreach ($departments as $department)
                    <div class="flex border-b-2 gap-1 border-gray-300 pb-1 items-center">
                        <p class="w-[100%] overflow-hidden text-nowrap">{{$department->name}}</p>
                        @if ($department->id != 1)
                            <a href="{{ route('edit-department', $department->id) }}"><x-button class="!bg-blue-600 !py-[0.2rem] mt-1 text-white w-8">✎</x-button></a>
                            <form action="{{ route('edit-department', $department->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
